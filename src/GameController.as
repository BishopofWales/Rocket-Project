...
package
{
	...
	{	
		...
		public function startGame()
		{
			...
			inputBoxes[1].text = 2357
			inputBoxes[2].text = 717000
			inputBoxes[3].text = 0
		...
		private function update(evt:Event)
		{
			...
			if(thrustLevel == 0)
			{
				...
				if(rVectorAngle < emptyFocciAngle) velocityAngle = (Math.PI-Math.abs(rVectorAngle-emptyFocciAngle))/2 + emptyFocciAngle +Math.PI;
				if(rVectorAngle > emptyFocciAngle) velocityAngle = (Math.PI-Math.abs(rVectorAngle-emptyFocciAngle))/2 + rVectorAngle + Math.PI;
